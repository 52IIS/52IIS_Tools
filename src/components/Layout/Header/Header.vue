...
import { ToolsInfo } from '@/menu/tools.type.ts';

import router from '@/router';
// const isNavDrawer = ref(false)
const loading = ref(false)
const options = ref<ToolsInfo[]>([])
const selectedOption = ref<ToolsInfo | null>(null)
//store
...
const createUrlShortcut = async () => {
  try {
    ...
    a.download = '52IIS-Tools-Web.url';
    a.click();
  } catch (error) {
...
<template>
  <header class="h-24 w-full flex justify-between pt-2 pb-2 c-xs:h-16 c-xs:border-b-[1px] items-center">
      ...
      <div class="c-xs:w-[85%] w-full mr-3">
        ...
        >
          <el-option
            ...
            @keyup.enter="optionClick(item.url)"
          >
          </el-option>
        </el-select>
      </div>
    ...
    <div class="flex w-full md:w-auto md:block c-xs:w-auto">
      <ul class="flex flex-col justify-end items-center mt-4 md:flex-row md:mt-0 c-xs:mt-0">
        <!-- about -->
        ...
        <li class="ml-3">
          <el-tooltip
              ...
              placement="bottom"
            >
            <a href="https://github.com/52IIS/52IIS_Tools" target="_blank">
